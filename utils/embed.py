...
class Embed:
    ...
    def splatoon2(title: str, description: str, color: int = 0xF03C78) -> disnake.Embed:
        ...
        return Embed.make_embed(
            color, f"<:Splatoon2:1036691271076560936> {title}", description
        ).set_footer(
            text="Données provenant de l'API du site Splatoon2.ink",
            icon_url="https://i.imgur.com/nvxf5TK.png",
        )
    ...
    def splatoon3(title: str, description: str, color: int = 0xEBEB3F) -> disnake.Embed:
        ...
        return Embed.make_embed(
            color, f"<:Splatoon3:1036691272871718963> {title}", description
        ).set_footer(
...
class RotationsEmbed:
    ...
    def get_splatoon2_embed(
        self, data: dict, number: int = 0, title: str = "Rotation actuelle"
    ) -> disnake.Embed:
        translation = json.load(open("./data/s2/translation.json"))

        match_pro = translation["rules"][data["gachi"][number]["rule"]["key"]]["name"]
        match_ligue = translation["rules"][data["league"][number]["rule"]["key"]][
            "name"
        ]
    ...
    def get_splatoon3_embed(
        ...
        if data["regularSchedules"]["nodes"][number]["regularMatchSetting"] is None:
            ...
            embed.add_field(
                ...
                value="- Aléatoire"
                if str(data["currentFest"]["tricolorStage"]["name"]).lower() == "random"
                else f"- {translation['stages'][data['currentFest']['tricolorStage']['id']]['name']}",
                inline=False,
            )
    ...
    def get_splatoon2_salmon_embed(
        self, data: dict, number: int = 0, title: str = "Rotation actuelle"
    ) -> disnake.Embed:
        if number == 2:
        ...
        weapons = [
            translation["weapons"].get(weapon["id"], {}).get("name", "Aléatoire")
    ...
    def get_splatoon3_salmon_embed(
        self,
        data: dict,
        gear_data: dict,
        number: int = 0,
        title: str = "Rotation actuelle",
    ) -> disnake.Embed:
        regular_schedules = data["regularSchedules"]["nodes"]
        big_run_schedules = data["bigRunSchedules"]["nodes"]

        now = (
            datetime.now(self.__timezone)
            if number == 0
            else self.__timezone.localize(
                datetime.strptime(
                    regular_schedules[number]["endTime"], "%Y-%m-%dT%H:%M:%SZ"
                )
            )
        )
        start_time = self.__timezone.localize(
            datetime.strptime(
                regular_schedules[number + 1]["startTime"], "%Y-%m-%dT%H:%M:%SZ"
            )
        )

        if (now - start_time) >= timedelta(hours=0) or not big_run_schedules:
            return self.__generate_splatoon3_salmon_embed(
                regular_schedules[number],
                gear_data,
                f"<:SalmonRun:1036691274415231006> {title}",
            )

        title = (
            "<:bigrun:1050787966794080379> Un Big Run fait des vagues !"
            if title == "Une nouvelle rotation est disponible !"
            else f"<:bigrun:1050787966794080379> {title}"
        )
        return self.__generate_splatoon3_salmon_embed(
            big_run_schedules[0], gear_data, title
        )

    def __generate_splatoon3_salmon_embed(
        self, data: dict, gear_data: dict, title: str
    ) -> disnake.Embed:
        translation = json.load(open("./data/s3/translation.json"))
        ...
        embed = (
            ...
            .set_image(data["setting"]["coopStage"]["image"]["url"])
            .add_field(
                name="🗺️ Map",
    ...
    def get_defi_oeuf_sup_embed(self, data: dict, number: int = 0) -> disnake.Embed:
        ...
        start_time = self.__timezone.localize(
            datetime.strptime(data["startTime"], "%Y-%m-%dT%H:%M:%SZ")
        )
        ...
        if (now - start_time) >= timedelta(hours=0):
            return (
                ...
                .add_field(
                    ...
                    value="\n".join(
                        [
                            f"- {translation['weapons'][weapon['__splatoon3ink_id']]['name']}"
                            for weapon in data["setting"]["weapons"]
                        ]
                    ),
                    inline=False,
                )
    ...
    def get_event_schedules_embed(self, data: dict, number: int = 0) -> disnake.Embed:
        ...
        if (
            ...
            or len(event_schedules[number].get("timePeriods", [])) == 0
        ):
            return Embed.splatoon3(
        ...
        embed = Embed.splatoon3(
            ...
            description=str(translation["events"][leagueMatchEvent["id"]]["regulation"])
            .replace("<br />", "\n")
            .replace("・", "- "),
            color=0xF02D7D,
        ).add_field(
        ...
        for period in data["timePeriods"]:
            embed.add_field(
                name="**📅 Créneau suivant**"
                if period == data["timePeriods"][0]
                else "<:invisible:1137708029417099274>",
