...
async def make_api_request(
    ...
    """
    merged_headers = (
        {**DEFAULT_HEADERS, **headers} if headers else DEFAULT_HEADERS.copy()
    )

    try:
        async with aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(timeout), headers=merged_headers
        ) as session:
...
async def update_data_if_needed(url: str, path: str) -> None:
    ...
    if not request:
        Logger.fail(
            f'Unable to check whether "{path}" file should be updated.', "update"
        )
        return
    ...
    if DeepDiff(data, request, ignore_string_case=True) != {}:
        ...
        with open(path, "w") as json_file:
            ...
            Logger.success(
                f'The "{path}" file has been successfully updated!', "update"
            )
