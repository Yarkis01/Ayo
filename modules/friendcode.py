...
class FriendCodeModule(commands.Cog):
    ...
    @_ca_command.sub_command(
        name="visibilite",
        description="Permet de sélectionner l'accès aux codes de vos amis.",
    )
    async def _public_access_command(
        ...
        allow: str = commands.Param(
            name="visibilite",
            description="Choisissez la visibilité des codes de vos amis",
            choices={"Publique": "True", "Privée": "False"},
        ),
    ) -> None:
        await inter.response.defer(ephemeral=True)

        allow = ast.literal_eval(allow)
        await self.__db.update_document(
            Collections.FRIEND_CODE, {"uid": inter.author.id}, {"publicAccess": allow}
        )
    ...
    @_ca_command.sub_command(
        name="ajouter",
        description="Permet d'ajouter ou de modifier un de ses codes amis",
    )
    async def _add_command(
        self,
        inter: disnake.CommandInteraction,
        code_type: str = commands.Param(
            name="type",
            description="Choisissez le type de code ami que vous souhaitez ajouter ou modifier",
            choices=CHOICES,
        ),
        ...
        if not self.__code_checker.check(code_type, code):
            await inter.send(
                embed=Embed.error(
                    ":x: Code ami invalide",
                    "Le code ami que vous avez entré n'est pas valide.",
                )
            )
            return
        ...
        await self.__db.update_document(
            ...
            {"uid": inter.author.id},
            {f"friendCode.{code_type}": self.__code_checker.format(code_type, code)},
        )
        await inter.send(
            embed=Embed.success(
                ":white_check_mark: Code ami ajouté",
                "Votre code ami a bien été ajouté.",
            )
        )

    @_ca_command.sub_command(
        name="supprimer", description="Permet de supprimer un de ses codes amis"
    )
    async def _del_command(
        ...
        code_type: str = commands.Param(
            name="type",
            description="Choisissez le code que vous souhaitez supprimer",
            choices=CHOICES,
        ),
    ) -> None:
        await inter.response.defer(ephemeral=True)
        ...
        await self.__db.update_document(
            Collections.FRIEND_CODE,
            {"uid": inter.author.id},
            {f"friendCode.{code_type}": None},
        )
        await inter.send(
            embed=Embed.success(
                ":white_check_mark: Code ami supprimé",
                "Votre code ami a bien été supprimé.",
            )
        )

    @_ca_command.sub_command(
        name="liste",
        description="Permet de voir ses codes amis ou ceux d'un autre utilisateur",
    )
    async def _show_command(
        ...
        member: disnake.User = commands.Param(
            ...
            name="membre",
            description="Utilisateur dont vous souhaitez voir les codes amis.",
        ),
    ) -> None:
        await self.show_friend_code(inter, member)

    @commands.user_command(name="Afficher Codes Amis", dm_permission=False)
    async def _show_user_command(
        self, inter: disnake.UserCommandInteraction, member: disnake.User
    ) -> None:
        await self.show_friend_code(inter, member)

    async def show_friend_code(
        self, inter: disnake.Interaction, member: disnake.User
    ) -> None:
        ...
        if member.bot:
            await inter.send(
                embed=Embed.error(
                    ":x: Code Ami",
                    "Vous ne pouvez pas afficher les codes amis d'un bot",
                )
            )
            return
