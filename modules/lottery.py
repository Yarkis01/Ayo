...
class LotteryModule(commands.Cog):
    ...
    @_lottery_command.sub_command(
        name="capsulomate", description="Obtiens les probabilités du Capsulomate"
    )
    ...
    @_lottery_command.sub_command(
        name="boîte-mystère", description="Obtiens les probabilités des boîtes mystères"
    )
    async def _mystery_box(
        self,
        inter: disnake.CommandInteraction,
        catalog: str = commands.Param(
            name="catalogue",
            choices={"Initial": "catalogue", "Bonus": "catalogue_bonus"},
        ),
    ) -> None:
        await inter.send(
            file=disnake.File(f"./data/s3/lottery/boite-mystere_{catalog}.png")
        )

    @_lottery_command.sub_command(
        name="saison",
        description="Obtiens les titres et les bannières rares de chaque saison",
    )
    async def _season_command(
        self,
        inter: disnake.CommandInteraction,
        season: str = commands.Param(name="saison"),
    ) -> None:
        ...
        if not data:
            await inter.send(
                embed=Embed.error(
                    ":x: Saison invalide",
                    "Merci de bien vouloir préciser une saison valide.",
                )
            )
            return
        ...
        await inter.send(
            ...
            .set_image(url=data["reward"]["banner"])
        )
