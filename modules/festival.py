...
class FestivalModule(commands.Cog):
    ...
    @commands.slash_command(
        name="festival",
        description="Permet d'obtenir des informations sur un festival (Splatoon 3)",
        dm_permission=False,
    )
    async def _festival_command(
        self,
        inter: disnake.CommandInteraction,
        festival: str = commands.Param(
            name="festival",
            description="Choisissez le festival sur lequel vous souhaitez obtenir des informations",
        ),
    ) -> None:
        ...
        if not data:
            await inter.send(
                embed=Embed.error(
                    ":x: Festival invalide",
                    "Merci de bien vouloir préciser un festival valide.",
                )
            )
            return
        ...
        embed = (
            ...
            .set_thumbnail(url=data["winner"]["image"])
            .set_image(url=data["image"])
        )

        for team in data["team"]:
            embed.add_field(
                name=f"🏆 {team['name']} l'emporte !"
                if team["name"] == data["winner"]["team"]
                else team["name"],
...
{f'- <:tricolore:1134914815375183882> Match Tricolore: **{team["result"]["tricolore"]}**%' if team['result']['tricolore'] else ''}""",
    ...
    async def _festival_autocomplete(
