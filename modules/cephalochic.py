...
class CephalochicModule(commands.Cog):
    ...
    def generate_splatoon2_translation_string(self, gear: dict, translation: dict) -> str:
    ...
    async def _s3_update_loop(self) -> None:
        if (self.__s3_next_rotation - self.__timezone.localize(datetime.utcnow())) >= timedelta(hours = 0):
            return
    ...
    async def _s2_update_loop(self) -> None:
        ...
        data = await make_api_request(f"{self.__config.splatoon2_api}/merchandises.json")
        if not data:
    ...
    @_cephalochic_command.sub_command(name = "splatoon3", description = "Permet d'avoir les équipements actuels de la boutique Céphalochic. (Splatoon 3)")
