...
class OdysseeView(disnake.ui.View):
    ...
    @disnake.ui.button(
        label="Afficher le tableau de données",
        emoji="<:odyssee:1038067635839057920>",
        style=disnake.ButtonStyle.blurple,
    )
    async def show_table_button(
...
class OdysseeModule(commands.Cog):
    ...
    @commands.slash_command(
        name="odyssee",
        description="Permet d'obtenir des informations sur l'odyssée d'Omar",
        dm_permission=False,
    )
    async def _odyssee_command(
        self,
        inter: disnake.CommandInteraction,
        voyage: str = commands.Param(
            name="voyage",
            description="Sur quelle voyage d'Omar voulez vous avoir des informations ?",
        ),
    ) -> None:
        ...
        if not data:
            await inter.send(
                embed=Embed.error(
                    ":x: Voyage invalide",
                    "Merci de bien vouloir préciser un voyage valide.",
                )
            )
            return
        ...
        await inter.send(
            ...
            .set_thumbnail(data["reward"]["image"]),
            view=view,
        )
        view.message = await inter.original_message()

    @_odyssee_command.autocomplete("voyage")
    async def _odyssee_autocomplete(
        ...
        return [
            ...
            if string.lower() in voyage["name"].lower()
        ][:25]
