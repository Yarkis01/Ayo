...
class BoxModule(commands.Cog):
    ...
    async def generate_rotation(self, game: int) -> tuple:
        if game == 3:
            return random.SystemRandom().choice(self.__s3_rules), random.SystemRandom().choice(self.__s3_stages)
        else:
            return random.SystemRandom().choice(self.__s2_rules), random.SystemRandom().choice(self.__s2_stages)
    ...
    async def _bo_command(
        ...
        number: commands.Range[int, 1, 10] = commands.Param(name = "nombre", description = "Nombre de parties que vous voulez jouer"),
        game: int = commands.Param(name = "jeu", description = "Quel jeu va être utilisé pour effectuer ses parties ?", choices = {"Splatoon 2": 2, "Splatoon 3": 3}, default = 3)
    ) -> None:
        ...
        while i != number:
            ...
            if (number > 6 and rules_count[rule] > 3) or (number <= 6 and rules_count[rule] > 2) or stages_count[stage] > 2:
                continue
